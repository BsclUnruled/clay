...
use std::{cell::Cell, collections::LinkedList, str::FromStr, vec};
...
impl Parser{
    pub fn new(code: &str) -> Self {
        Self {
            code:code.chars().collect::<Vec<char>>().into_boxed_slice(),
            index: 0.into(),
        }
    }
    ...
    fn peek(&self) -> Option<char> {
        match self.code.get(self.index.get()) {
            Some(c) => Some(*c),
            None => None,
        }
    }
    ...
    fn parse_symbol(&self, sym: char) -> Return {
        ...
        loop{
            match self.peek() {
                Some(c) =>{
                    if SYMS.contains(&c) {
                        hc.push(self.next().unwrap());
                    }else{
                        break Ok(Token::Symbol(hc.iter().collect::<String>()));
                    }
                }
                None => {
                    return Err(format!("Unexpected end of code(from parse_symbol)"));
                }
            }
        }
    }
